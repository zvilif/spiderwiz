...
class MyConfig extends ZConfig{
    ...
    /**
     * Figure out ping rate for a channel or server channel from configuration parameters
     * @return 
     */
    static long getPingRate(ZDictionary configParams) {
        int rate = ZUtilities.parseInt(configParams.get(CoreConsts.DataChannel.PINGRATE));
        return (rate > 0 ? rate : CoreConsts.DataChannel.DEFAULT_PINGRATE) * ZDate.SECOND;
    }
