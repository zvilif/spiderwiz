...
class AlertMail {
    ...
    public AlertMail() {
        ...
        if (mailConfig != null) {
            ...
            if ((customClass = configParams.get(CoreConsts.AlertMail.CLASS)) != null) {
                Constructor<?> c;
                try {
                    obj = ZMail.class.cast(Class.forName(customClass).newInstance());
                } catch (ClassNotFoundException | InstantiationException | IllegalAccessException ex) {
                    Main.getInstance().sendExceptionMail(ex,
                        String.format(CoreConsts.AlertMail.MAIL_CLASS_FAILED, customClass, ex.toString()), null, true);
                }
        ...
        String stackTrace, ZDate eventTime, boolean alert)
    {
        ...
        try {
            zmail.configure(config.getProperty(MyConfig.MAIL_SYSTEM));
