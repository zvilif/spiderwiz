...
public abstract class Channel extends Dispenser<String> implements Runnable {
    ...
    /**
     * Returns the time interval that buffered output to this channel should be flushed.
     * <p>
     * By default buffered output to this channel will be flushed every second. Override this method to provide a different
     * value in milliseconds. Return zero if flushing shall never take place.
     * @return  the time interval in milliseconds that buffered output to this channel should be flushed, or zero if flushing
     * is not necessary.
     */
    ...
    private boolean send(String line, boolean flush) {
        ...
        try {
            ...
            if (flush || getFlushInterval() > 0 && lastFlush.elapsed() >= getFlushInterval()) {
                flush();
                lastFlush = ZDate.now();
                flushed = true;
            }
