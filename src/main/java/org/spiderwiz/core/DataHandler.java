...
final class DataHandler extends ChannelHandler {
    ...
    private class ChannelMonitor extends ZThread {
        private static final int MONITOR_DEFAULT = ZDate.MINUTE;
        ...
        protected long getLoopInterval() {
            ...
            return getLoginSent() != null ? RELOGIN_DEFAULT : MONITOR_DEFAULT;
        }
    }
    ...
    boolean initClientChannel(String defString, int n) {
        try {
            ...
            if ((customClass = configParams.get(CoreConsts.Channel.CLASS)) != null) {
                ...
                try {
                    channel = (Channel)Class.forName(PluginConsts.SpiderAdmin.SPIDERADMIN_CLASS).newInstance();
                } catch (Exception ex) {
    ...
    private void postLogin() throws Exception {
        ...
        boolean rawLog =
            ...
            > 0;
        sequenceManager.onLogin(logicalCompression, fullLog, rawLog);
    }
    ...
    private void processReset(String s[]) throws Exception {
        ...
        requestedObjects.fromString(objectList, Main.getMyConfig().isHubMode() ? null : DataManager.getInstance().getProducedObjects());
        // Continue only if list of requested objects is not empty
        ...
        // If this is a full dailyReset request we need to store application data and add the origin node to the connected node
        // list.
        if (originUUID != null && Hub.getInstance().isMe(originUUID) < 0) {
            connectedNodes.add(originUUID);
            Hub.getInstance().handleResetRequest(originUUID, requestedObjects.getAsString(), requestingAppName, requestingAppVersion,
        ...
        Hub.getInstance().broadcastResetRequest(
            requestedObjects.getAsString(), requestorUUID, destUUID, ts, seq, deployTime, originUUID, applicationParams,
     ...
     * @param requestorUUID     UUID of the node requesting the dailyReset. Null if not known.
     * @param targetUUID        UUID of the node from which the dailyReset is requested, or null if all
    ...
    private boolean processCommand(String command) throws Exception {
        ...
        try {
            if ((   Hub.getInstance().isForMe(components.destinations) < 0 ||
                    !DataManager.getInstance().isConsumingObject(objCode) ||
                    !DataManager.getInstance().processCommand(
                        prefix, objCode, components.keys, components.fields, components.origin, components.destinations, this,
                        components.expandedCommand, components.timestamp, components.ackSeq, false, command.length()
    ...
    void transmitCommand(String prefix, String objCode, ZDate ts, String objectKeys, String objectValues, UUID originUUID,
        ...
        if (getSequenceManager() != null)
            sequenceManager.transmitCommand(
                prefix, objCode, ts, objectKeys, objectValues, originUUID, destinations, objSeq, ackSequence, moderated);
    }
     ...
     * side it is a procedure for conflict alert.
     */
    private void login() {
        if (!producer) {
