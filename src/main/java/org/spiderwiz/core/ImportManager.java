...
class ImportManager {
        ...
        synchronized (importChannels) {
            for (int i = 1; i < MAX_IMPORTS; i++) {
                ...
                if (def != null && channel == null) {
                    channel = Main.getInstance().createImportHandler(i);
                    channel.init(def, i);
    ...
    void dropNode(UUID uuid) {
        ...
        try {
            for (ImportHandler channel : importChannels.values()) {
                if (channel.getAppUUID().equals(uuid))
                    removed.add(channel);
            }
        } finally {
     ...
     * @return 
     */
    boolean transmitObject(DataObject obj) throws Exception{
        boolean sent = false;
        importChannels.lockRead();
        try {
            for (ImportHandler channel : importChannels.values()) {
                sent |= channel.transmitObject(obj);
            }
        } finally {
            importChannels.unlockRead();
        }
