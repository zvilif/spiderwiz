 ...
 * should call {@link #cleanup()} . For example:
 *  <a name="InitExample"></a>
    <pre>
    ...
    * <p>
    * There are two ways to provide your own initialization code. The first is to override
    * {@link #init()} and write your code after you call {@code super.init()}. This code will be executed after the default
    * framework initialization, when communication channels have been established and are opened for data flow. The second way is to
    * override {@link #preStart()}, which is called once the
    ...
    * <p>
    * The framework main ingredient is the <em><b>data object</b></em>, implemented as a class that extends {@link DataObject}.
    * By convention, <em>data objects</em> are implemented in an organization-shared Java Class Library. When
    ...
    * implementation code specific to your application. These are optional for object types you produce and essential for object types
    * you consume, as you need to provide event handling code. To register your implementation class extensions override
    ...
    * <p>
    * See the description below for other methods that are used less frequently.
 */

@WizMain
public abstract class Main {
    private static final String CORE_VERSION = "Z2.30"; // Version Z2.30: First published version.
    ...
    protected static class ObjectCodes {
         ...
         * {@link DataObject#importObject(java.lang.String, org.spiderwiz.core.ImportHandler, org.spiderwiz.zutils.ZDate) 
         ...
         * <p>
         * Use this when you want to delegate the action of
         * {@link org.spiderwiz.core.DataObject#exportObject(org.spiderwiz.core.ImportHandler)
    ...
    private final String rootDirectory, configFileName, appVersion;
     ...
     * @param rootDirectory     the application root directory. An empty string refers to the project base directory (${basedir}).
     ...
     */
    public Main(String rootDirectory, String configFileName, String appName, String appVersion) {
        this.rootDirectory = rootDirectory;
        rootFile = new File(rootDirectory != null && !rootDirectory.isEmpty() ? rootDirectory : "/");
        ...
        queryManager.init();
        dataManager = new DataManager(eventDispatcher, queryManager);
     ...
     * </ul>
     * <p>
     * You can provide custom initialization code by overriding this method and putting your code after
     * calling its {@code super} instance. Alternatively you can override
     * {@link #preStart()}, which is called once the application's configuration file has been loaded and the logging mechanism
     * has been established, but before communication channels are established and data starts to flow.
     *
    ...
    public synchronized boolean init() {
        ...
        myObject = this;
        try {
            rootFolder = MyUtilities.prefixDevice(rootDirectory);
            ...
            if (!config.init(rootFolder + configFileName) ) {
                System.out.printf(CoreConsts.NO_CONFIG_FILE, rootFolder + configFileName);
                System.out.println();
                return false;
            }
            startLogging(true);
     ...
     * <p>
     * This method clears the entire <em>data object tree</em> and broadcasts a request to all peer applications to reset the objects
     * it consumes. The method will be called daily at the time set by [{@code start of day}] property of the
     * <a href="doc-files/config.html">application's configuration file</a> (unless you override
     * {@link #runDailyTask() runDailyTask()}).
     * <p>
     * You can override this method if you don't want to reset the object tree daily, or if you want to do something more (in which
     * case you shall also call the {@code super} instance of this method).
     */
    ...
    /**
     * Override this method to add more rows to factoryMap table. Do not forget to call super.addDataClass() in order to keep
     * the predefined rows.
     * @param factoryMap
     */
    ...
    final DataObject createDataObject(String objectType)  {
        try {
            ...
            if (dataClass != null) {
                DataObject object = (DataObject)dataClass.newInstance();
                object.setOriginUUID(appUUID);
                return object;
            }
            throw new NoSuchElementException(String.format(CoreConsts.INVALID_OBJECT_TYPE, objectType));
        } catch (InstantiationException | IllegalAccessException | NoSuchElementException ex) {
     ...
     */
    public final synchronized DataObject getRootObject() {
        return dataManager == null ? null : dataManager.getRootObject();
    }

    /**
     * If included in the application's classpath, Add XAdminQuery to the factory map
     * @param factoryMap 
     */
    ...
    private OpResultsEx createOpResults(String status){
        return new OpResultsEx(status);
    }
     ...
     * @return {@link org.spiderwiz.admin.xml.OpResults OpResults} object (will be documented in a future release).
     */
     ...
     * <a href="doc-files/config.html">application's configuration file</a>. By default it resets communication monitoring counters
     * and calls {@link #reset() reset()}. You can override it to do something else or something more.
     */
    ...
    /**
     * Called when a <em>data objects</em> of a specific type need a reset.
     * <p>
     ...
     * but if a data object is defined {@link org.spiderwiz.core.DataObject#isDisposable() disposable} the application may
     * need to do a reset from an external source (e.g. a database). To do that, override this method.
     ...
     */
    protected void onObjectReset(Resetter resetter) {
    }
     ...
     * will be created.
     * @param number    the number that identifies the import channel in [import-n] property of the
     * <a href="doc-files/config.html">application's configuration file</a>.
     * @return a new instance of {@link org.spiderwiz.core.ImportHandler ImportHandler} or a subclass of it.
     * @see ImportHandler
     */
    protected ImportHandler createImportHandler(int number) {return new ImportHandler(number);}
     ...
     * </ul>
     * @return an object containing application information. Will be documented in detail in a later version.
     */
    public ApplicationInfoEx getApplicationInfo() {
        return new ApplicationInfoEx(getAppName(), getAppVersion(), getCoreVersion(), ZUtilities.getMyIpAddress(),
            deployDate.format(ZDate.FULL_DATE), getStatus(), 0, 0, getRootFolder(),
            getAppUUID().toString());
    }
     ...
     * with data by overriding {@link #customAdminService(java.lang.String) customAdminService()}.
     * @return an object containing page layout information. Will be documented in detail in a later version.
     */
     ...
     * @return an object containing table structure information. Will be documented in detail in a later version.
     */
    public TableInfoEx getServerInfoTableStructure() {
     ...
     *
     * @param row an object to fill in data for one row of an administration table. Will be documented in detail in a later version.
     */
    protected void addServerInfoRowData(TableDataEx.RowDataEx row) {
     ...
     * @return an object containing table data. Will be documented in detail in a later version.
     */
    public TableDataEx getServerInfoTableData() {
        TableDataEx td = new TableDataEx();
     ...
     * @return an object containing table structure information. Will be documented in detail in a later version.
     */
    public TableInfoEx getApplicationsTableStructure(String tableTitle, String tableService) {
     ...
     * @return an object containing table structure information. Will be documented in detail in a later version.
     */
    public TableInfoEx getConnectedNodesTableStructure(String tableTitle, String tableService) {
     ...
     * @return an object containing table structure information. Will be documented in detail in a later version.
     */
    public TableInfoEx getImportsTableStructure(String tableTitle, String tableService) {
     ...
     * @return an object containing table data. Will be documented in detail in a later version.
     */
    public TableDataEx getApplicationsTableData(String userID) {
     ...
     * @return an object containing table data. Will be documented in detail in a later version.
     */
    public TableDataEx getConnectedNodesData(boolean consumers, String userID) {
     ...
     * @return an object containing table data. Will be documented in detail in a later version.
     */
    public TableDataEx getImporNodesData() {
     ...
     * customized through {@link #getPageInfo() getPageInfo()}. Override this method to assign actions to buttons and fill
     * custom tables with data.
     * @param serviceTag    the tag assigned to your custom element.
     * @return an object that shall be {@link org.spiderwiz.admin.xml.OpResults} if the tag identifies a button or
     * {@link org.spiderwiz.admin.xml.TableData} if it identifies a table. Both will be documented in detail in a later version.
     */
    public Object customAdminService(String serviceTag) {
        return null;
    }
}
