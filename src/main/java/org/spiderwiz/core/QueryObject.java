 ...
 * <p>
 * Note that both the inquirer and the responder of a query object must register as <b>both a
 * {@link org.spiderwiz.core.Main#getProducedObjects() producer} and a {@link org.spiderwiz.core.Main#getConsumedObjects() consumer}</b>
 * of the object, because it is a producer of the query and a consumer of the response or vis-versa.
 * <p>
...
public abstract class QueryObject extends DataObject{
    ...
    /**
     * Posts the query to specific destination(s).
     * <p>
     ...
     * were activated. Partial reply is when {@link #onReplyNext()} was activated but {@link #onReplyEnd()} was not, or a reply was
     ...
     * equals to the value set when {@link #post(long) post()} was called.
     * <p>
     ...
     * zero.
     * <p>
     * The method returns {@code true} if the response item is committed successfully. If the query is not active any more
    ...
    /**
     * Returns {@code true} if this is un urgent query.
     * <p>
     * Override this method to mark the query as <em>urgent</em>. Urgent queries have priority over other items circulated by the
     * framework. The default implementation returns {@code false}.
     *
     * @return true if and only if this is un urgent query.
     */
    ...
    /**
     * Returns the streaming rate of a a multi-item stream response.
     * <p>
    ...
    void setDestinations(Collection<UUID> destinations) {
    ...
    final synchronized void activate() {
        if (isComplete())
            activated = null;
        else if (queryState == QueryState.QUERY || queryState == QueryState.NEXT)
            activated = ZDate.now();
    }
    ...
    final boolean abortQuery() throws Exception {
        ...
        if (Hub.getInstance().isMe(getOriginUUID()) > 0)
            propagate(false, destinations, null);
        return true;
    }
