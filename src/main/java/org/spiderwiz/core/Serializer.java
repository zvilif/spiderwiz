...
class Serializer {
    ...
    private static final HashMap<Character, Character> unscapeMap = new HashMap<Character, Character>() {
        {
            for (Entry<Character, Character> e : escapeMap.entrySet()) {
    ...
    private Class serializeClass(StringBuilder sb, Object obj, Class cl, boolean enclose) throws Exception
    {
        ...
        if (cl == null)
            return null;
        boolean xml = cl.isAnnotationPresent(XmlType.class);
    ...
    private Class getSerializableClass(Class cl) {
        ...
        while (
            ...
            !cl.isAnnotationPresent(XmlType.class)
        ) {
            cl = cl.getSuperclass();
            if (cl == null)
                return null;
        ...
        boolean ignoreZone) throws Exception
    {
        ...
        int startLength = sb.length();
        for (Object val : vals) {
            sb.append(sb.length() == startLength ? LIST_START : LIST_SEPARATOR);
        ...
        throws Exception
    {
        ...
        int startLength = sb.length();
        for (Object val : vals) {
            sb.append(sb.length() == startLength ? MAP_START : LIST_SEPARATOR);
            serializeFieldObject(sb, val, parameterClass, parameterClass, format, precision, ignoreZone);
        }
        sb.append(MAP_END);
    }
    ...
    private int deserializeClass(Object obj, Class objClass, String[] vals, int offset) throws Exception
    {
        if (objClass == null)
            return offset;
        boolean xml = objClass.isAnnotationPresent(XmlType.class);
        ...
        throws Exception
    {
        ...
        String[] elements = splitFields(val.substring(1, end), LIST_SEPARATOR);
        ...
        throws Exception
    {
        ...
        if (val.charAt(0) != LIST_START || val.charAt(end) != LIST_END)
            throw new ParseException(String.format(CoreConsts.Serializer.LIST_PARSE_ERROR, val), 0);
        return (List)deserializeCollection(val.substring(1, end), cl, type, outer, format, ignoreZone);
    }
        ...
        throws Exception
    {
        ...
        return (Set)deserializeCollection(val.substring(1, end), cl, type, outer, format, ignoreZone);
    }
    ...
    /**
     * Return a string that is a concatenation of the escaped version of all the parameter with 'delimit' as a
     ...
     */
    private void removeTrailingDelimiters(StringBuilder sb, int indexFrom, char delimiter) {
        int i;
    ...
    private void decompressMap(StringBuilder sb, String previous, String delta) {
        ...
        int sbl = sb.length();
        for (Entry<String,String> e : dm.entrySet()) {
            String key = e.getKey();
