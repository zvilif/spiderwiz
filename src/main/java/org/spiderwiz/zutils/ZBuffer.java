 ...
 * <p>
 * The classes uses a {@link java.util.LinkedList} to hold a queue of elements and manage their processing. Typically the user of the
 ...
 * <p>
 * There is a {@link #ZBuffer(org.spiderwiz.zutils.ZDispenser, boolean) constructor} version that lets you defining the buffer as
 * <em>multi-threaded</em>, in which case multiple threads, as many as available CPUs, will be invoked to pull buffer items and
 * process them.
 * <p>
 ...
 * @param <T> type of elements manages by the buffer.
 * @see ZDispenser
 */
public class ZBuffer<T> implements Runnable {
     ...
     * <em>user-dispensed</em>, i.e. the user needs to call {@link #pull(boolean) pull()} to process buffer elements.
     * <p>
     * The {@code multithread} parameter determines whether or not the constructed buffer is single or multi thread. In a single-thread
     * buffer only one thread is assigned to pull elements from the buffer and process them. Multi-thread buffer invokes several
     * threads, as many as the available CPUs, to process buffer elements. Note that a multi-thread buffer cannot guarantee
     * processing order.
     * @param dispenser     the buffer's dispenser, or null if this is a <em>user-dispensed buffer</em>.
     * @param multithread   if true then if dispenser is not null multi-thread buffer is constructed.
     */
    public ZBuffer(ZDispenser<T> dispenser, boolean multithread) {
     ...
     * <p>
     * Sets the maximum time in milliseconds a pull operation will wait on empty buffer until an element is available. If
    ...
    public final void execute(String backupFile) {
        ...
        if (!discardOnFull && dispenser != null) {
            for (int i = multithread ? Runtime.getRuntime().availableProcessors() : 1; --i >= 0;)
    ...
    /**
     * Inserts an urgent element to the the buffer.
     * <p>
